...
export default function ReportsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-8 font-sans">
      <h1 className="text-2xl font-semibold text-foreground  ">Reportes y métricas</h1>
        ...
        {preset === "custom" && (
          <div className="flex items-center gap-1.5">
            <input
              ...
              className="rounded border border-border px-2 py-1.5   bg-surface"
            />
      ...
      <div className="flex gap-4 border-b border-border text-sm  ">
        <button
          ...
          className={`border-b-2 pb-2 ${tab === "sales" ? "border-accent font-medium border-accent text-accent" : "border-transparent text-muted"}`}
        >
          Ventas
        </button>
        <button
          onClick={() => setTab("audit")}
          className={`border-b-2 pb-2 ${tab === "audit" ? "border-accent font-medium border-accent text-accent" : "border-transparent text-muted"}`}
        >
          Auditoría de cajas
        </button>
      </div>
      ...
      {tab === "sales" && (
        <div className="space-y-6">
          {summary.isLoading && <p className="text-sm text-muted">Cargando…</p>}
          {summary.data && (
            <>
              ...
              <section className="rounded-lg border border-border p-4  ">
                <h2 className="mb-3 text-sm font-medium text-foreground  ">Evolución de ventas</h2>
          ...
          <section className="rounded-lg border border-border p-4  ">
            <h2 className="mb-3 text-sm font-medium text-foreground  ">Medios de pago</h2>
            {payments.data && <PaymentMethodsChart breakdown={payments.data.breakdown} />}
          </section>

          <section className="rounded-lg border border-border p-4  ">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-sm font-medium text-foreground  ">Top 10 productos</h2>
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            ...
            <button onClick={handleExportPdf} className="rounded border border-border px-3 py-1.5  ">
      ...
      {tab === "audit" && (
        <div className="space-y-4">
          ...
          {shiftsHistory.isLoading && <p className="text-sm text-muted">Cargando…</p>}
...
function KpiCard({ label, value, negative }: { label: string; value: string; negative?: boolean }) {
  return (
    <div className="rounded-lg border border-border p-4  ">
      ...
      <p className={`mt-1 text-xl font-semibold ${negative ? "text-red-600 dark:text-red-400" : "text-foreground  "}`}>
        {value}
      </p>
    </div>
  );
